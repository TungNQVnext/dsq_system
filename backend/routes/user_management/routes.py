...
from routes.login.utils import hash_password
...
def get_current_user(
    ...
    try:
        ...
        if not user:
            ...
            raise HTTPException(status_code=401, detail="User not found")
        print(f"Found user via cookie: {user.username}, role: {user.role}")
        return user
...
def check_admin_permission(current_user: User = Depends(get_current_user)):
    ...
    print(f"Checking admin permission for user: {current_user.username}, role: {current_user.role}")
    ...
    print(f"Admin permission granted for user: {current_user.username}")
    return current_user
