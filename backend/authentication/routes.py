...
import datetime
from authentication.jwt_utils import create_access_token, get_current_user_from_cookie
...
def login(login_data: LoginInput, response: Response, db: Session = Depends(get_db)):
    ...
    # Set cookie HTTPOnly, session cookie (không set max_age/expires)
